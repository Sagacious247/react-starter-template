...
const cartSlice = createSlice({
  ...
  reducers: {
    ADD_TO_CART(state, action) {
        ...
        if(productIndex >= 0) {
            // Itel already exists in the cart
            // Increase the cartQuantiy
            state.cartItems[productIndex].cartQuantity += 1
            toast.info(`${action.payload.name} increased by one`, {position: "top-left"})
        } else {
            // Itel does not exists in the cart
            // Add item to cart
...
export const {CALCULATE_SUBTOTAL,CALCULATE_TOTAL_QUANTITY, SAVE_URL, ADD_TO_CART, DECREASE_CART, REMOVE_FROM_CART, CLEAR_CART} = cartSlice.actions
